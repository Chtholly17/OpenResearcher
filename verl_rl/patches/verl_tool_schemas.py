...
#
# MONKEY PATCH: verl/tools/schemas.py
# ------------------------------------
# Replaces: site-packages/verl/tools/schemas.py
#
# Changes vs upstream verl 0.7.0:
#   1. OpenAIFunctionPropertySchema.type: str -> str | list[str]
#      Supports JSON Schema array types like ["integer", "string"] used by
#      Nemotron tool definitions (e.g., browser.open's id parameter).
#   2. Added ConfigDict(extra="allow") to OpenAIFunctionPropertySchema and
#      OpenAIFunctionParametersSchema so extra fields (e.g., "default") are
#      preserved through Pydantic validation and included in model_dump().
#      Without this, the Nemotron chat template's render_extra_keys macro
#      cannot render <default>value</default> tags, causing prompt mismatch
#      between SFT training and RL rollouts.
#
# Apply:
